...
*   **目錄書籤**：點選「＋ 新增書籤」，在目前瀏覽作品前方插入可命名的分組標記；使用 `✎` 重新命名、`×` 刪除，拖曳書籤到作品或其他書籤之間即可移動，藍線標示放置位置。拖曳至目錄上下邊緣可捲動。
    *   書籤位置跟隨後方作品；篩選時仍保留標記，刪除對應作品後標記移至目錄底部。
    *   名稱與位置自動儲存，並包含在 JSON 匯入／匯出中；匯入舊版備份時，書籤清單設為空白。
    *   **邊界吸附**：最近完全捲出目錄上方的書籤顯示在頂部，下一個尚未進入可視區域的書籤顯示在底部。書籤開始進入畫面時，該書籤的固定提示會移除，改為正常顯示原本的書籤列。
    *   固定提示使用獨立空間，不遮住作品。上方沒有捲出的書籤時會收起提示列，滑回最上方不會留下多餘空白；底部保留提示空間以穩定切換邊界。點擊固定提示可捲動到原書籤位置，並支援鍵盤操作與減少動態效果偏好。
    *   目錄高度會扣除頁首占用的空間，並隨頁面捲動、視窗縮放調整，確保底部提示保留在視窗內。
